    ...
    public class BatSpawner : MonoBehaviour
    {
        ...
        private IEnumerator SpawnBats()
        {
            ...
            while (true)
            {
                if(_bats.Count == 0)
                {
                    GameObject batObject =  Instantiate(_batPrefab, _spanwerLocations[order % _spanwerLocations.Count].position, _batPrefab.transform.rotation, transform);
                ...
                else
                {
                    ...
                    bat.transform.position = _spanwerLocations[order % _spanwerLocations.Count].position;
